...
| No   | 処理内容               | URL<br/>（Front-end）    | 入力パラメータ                                               | controller        | service                          | URI<br/>（MS）                                   | 戻り値判定                                                   | レスポンスモデル             | 備考 |
| ---- | ---------------------- | ------------------------ | ------------------------------------------------------------ | ----------------- | -------------------------------- | ------------------------------------------------ | ------------------------------------------------------------ | ---------------------------- | ---- |
| 1    | 部屋情報マスタ取得処理 | RentalsRoom<br/> /Query  | ・nodeId：企業コード<br/> ・indexNo：0<br/> ・orderBy：'indexNo'<br/> ・ascending：true<br/> ・startIndex：0<br/> ・batchSize：0 | rentalsRoomQuery  | rentalsService.RentalsRoomQuery  | POST<br/> rentals-room/room/query                | ■オンライン正常<br/> ・statusCode:200･･･正常（応答コード：0）<br/> ・statusCode:200以外･･･異常（応答コード：statusCode）<br/> ■オンライン異常<br/> ・JSON変換エラー･･･異常（応答コード：-20）<br/> ・以外･･･異常（応答コード：-30）<br/> ■オフライン（Back-end→MS）<br/> ・通信異常･･･異常（応答コード：-10） | RentalsCommonResponseModel() |      |
| 2    | 部屋情報マスタ更新処理 | RentalsRoom<br/> /Update | ・nodeId：企業コード<br/> ・indexNo：インデックス番号<br/> ・room：部屋設定項目欄に設定した内容 | rentalsRoomUpdate | rentalsService.RentalsRoomUpdate | POST<br/> rentals-room/room                      | ■オンライン正常<br/> ・statusCode:200･･･正常（応答コード：0）<br/> ・statusCode:200以外･･･異常（応答コード：statusCode）<br/> ■オンライン異常<br/> ・JSON変換エラー･･･異常（応答コード：-20）<br/> ・以外･･･異常（応答コード：-30）<br/> ■オフライン（Back-end→MS）<br/> ・通信異常･･･異常（応答コード：-10） | RentalsUpdateResponseModel() |      |
| 3    | 部屋情報マスタ削除処理 | RentalsRoom<br/> /Delete | ・nodeId：企業コード<br/> ・indexNo：インデックス番号        | rentalsRoomDelete | rentalsService.RentalsRoomDelete | DELETE<br/> rentals-room/room/{nodeId}/{indexNo} | ■オンライン正常<br/> ・statusCode:200･･･正常（応答コード：0）<br/> ・statusCode:200以外･･･異常（応答コード：statusCode）<br/> ■オンライン異常<br/> ・JSON変換エラー･･･異常（応答コード：-20）<br/> ・以外･･･異常（応答コード：-30）<br/> ■オフライン（Back-end→MS）<br/> ・通信異常･･･異常（応答コード：-10） | RentalsCommonResponseModel() |      |
