 ...
 */
@RestController
@RequestMapping("RentalsRoomcourse")
public class RentalsRoomcourseController {
  ...
  @Autowired
  public RentalsRoomcourseController() {}
   ...
   * ルームコースアイテム取得
   *
   * @param model リクエスト内容[nodeId,chargeCode]
  ...
  public RentalsCommonResponseModel rentalsRoomcourseQuery(@RequestBody @Validated Map<String, Object> model,
		...
		paramReq.getRequestModel().setOrderBy("chargeCode");
...
//KSD V001.000 20230810 AE
		...
		// POST要求（取得）
		////////////////////////////////
		responseModel = rentalsService.RentalsRoomcourseQuery(paramReq, bodyReq,
...
// KSD V001.000 20230922 AE
   ...
   * ルームコースアイテム新規作成／更新
   *
   * @param model リクエスト内容[chargeCode,...,endDate]
   * @param errors バリデーションエラー内容
   * @param request HttpServletのリクエスト(FWが自動で設定)
   * @return 検索処理＋αレスポンス
   */
...
//KSD V001.000 20230816 AS
	  public RentalsUpdateResponseModel rentalsRoomcourseUpdate(@RequestBody @Validated Map<String, Object> model,
		  Errors errors, HttpServletRequest request, HttpServletResponse response) {
...
//KSD V001.000 20230816 AE
		...
		// POST要求（更新）
		////////////////////////////////
		responseModel = rentalsService.RentalsRoomcourseUpdate(model,
				messageSource, apiContext, accessToken, ELERAToken, loginUser.getBusinessUnitCdStr() + loginUser.getUserId(), loginUser.getPassWord());
...
// KSD V001.000 20230922 AE
  ...
  @ResponseBody
  public RentalsCommonResponseModel rentalsRoomcourseDelete(@RequestBody @Validated Map<String, Object> model,
		  Errors errors, HttpServletRequest request, HttpServletResponse response) {
		...
		// POST要求
		////////////////////////////////
		responseModel = rentalsService.RentalsRoomcourseDelete(model,
				messageSource, apiContext, accessToken, ELERAToken, loginUser.getBusinessUnitCdStr() + loginUser.getUserId(), loginUser.getPassWord());
