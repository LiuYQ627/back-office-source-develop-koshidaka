...
const storeGroupTwo = 'Reservation/FetchConfigurationRecursive'
const STORE_GROUP_2_MASTER_UPDATE = 'StoreGroup2Master/Update'
...
export default {
  ...
  methods: {
    ...
    async getStoreGroupMaster (reloadDataOnly = false) {
      ...
      try {
        let response = await axios.put(this.$i18n.t(`prop.url`) + storeGroupTwo, {
        ...
        switch (response.data.result.code) {
          ...
          default:
            ...
            this.globalErrorMapping2(response.data.result, code === -10 ? this.$t('F32252.E001') : this.$t('F32252.E002'))
        }
      } catch (error) {
    ...
    async handleFixedBtn () {
      ...
      await this.postStoreGroup2MasterUpdate()
    },
    ...
    async postStoreGroup2MasterUpdate () {
      let orderedList = []
      ...
      await axios.put(
        `${this.$i18n.t('prop.url')}${STORE_GROUP_2_MASTER_UPDATE}`,
        {STORE_GROUP_2: {...this.dataModel}},
        commonUtils.methods.getApiHeader({})
      ).then(async (response) => {
        switch (response.data.result.code) {
          case 0: // 0:正常
          ...
          default:
            const code = response.data.result.code
            this.globalErrorMapping2(response.data.result, code === -10 ? this.$t('F32252.E003') : this.$t('F32252.E006'))
        }
      }, async (error) => {
        console.error(error)
        await this.openPopupDialog({ mode: 3, messageCode: 'O00004.W010' })
    ...
    searchErrorMapping (result) {
      if (result.code === -99) {
        let errorMessage = result.errorMessageMap['storeCd'].toString().split(',').join('')
