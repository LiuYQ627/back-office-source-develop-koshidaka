...
const DEFAULT_CUSTOMER_FILE_ITEM_MODEL = {
  filename: null,
...
const DEFAULT_CUSTOMER_FILE_MODEL = {
  ksdtaikiseidougacartoonfilename: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename1: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename2: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename3: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename4: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename5: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename6: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename7: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename8: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename9: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL)),
  ksdtaikiseishigaimagefilename10: JSON.parse(JSON.stringify(DEFAULT_CUSTOMER_FILE_ITEM_MODEL))
}

export default {
  name: 'SelfPOSMasterSettingCustomerFilePreview',
  ...
  methods: {
    ...
    resolveFileSourceCommon (id) {
      if (this.customerFileModelCommon && id in this.customerFileModelCommon) {
        const metadata = this.customerFileModelCommon[id]
    ...
    resolveFileSourceIndividual (id) {
      if (this.customerFileModelIndividual && id in this.customerFileModelIndividual) {
        const metadata = this.customerFileModelIndividual[id]
        if (metadata.authenticatedUrl != null) {
          return metadata.authenticatedUrl
        } else {
          return null
    ...
    isImage (id) {
      return this.customerFileModelCommon[id].filename.toLowerCase().endsWith('.png')
    },
    loads3MasterFile () {
    ...
    loadFileDataModel () {
      ...
      if (targetCustomerFileModel != null) {
        ...
        this.settingItemFileNameOptions = loadFileSelection(this.customerFileModel)
          ...
          .then(response => {
            ...
            if (response.data.result.code === 0) {
              if (response.data.responseModel != null) {
                this.dataConfig.CUSTOMER_FILE_SETTINGS = setting.configurations.CUSTOMER_FILE_SETTINGS.value
    ...
    getS3FilesBase () {
      return new Promise((resolve, reject) => {
        if (this.dataConfig != null) {
          const targetS3BucketBaseFiles = Object.values(this.dataConfig.CUSTOMER_FILE_SETTINGS).reduce((prev, curr) => {
            if (curr !== '') {
              prev.push(curr)
            }
            return prev
    ...
    getS3Files () {
      return new Promise((resolve, reject) => {
        if (this.dataConfig != null) {
          const targetS3BucketFiles = Object.values(this.dataConfig.CUSTOMER_FILE_SETTINGS).reduce((prev, curr) => {
            if (curr !== '') {
              prev.push(curr)
            }
            return prev
