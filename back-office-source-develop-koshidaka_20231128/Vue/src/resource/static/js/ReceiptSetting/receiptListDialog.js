 ...
 * ---------+---------------------+----------+--------------------------------
 * 20230320  wangchunmei(Neusoft)  G001.00.0  issue課題#1586を対応します.
 * 20230322  wangchunmei(Neusoft)  G002.00.0  issue課題#1588を対応します.
 * 20230612  wangchunmei(Neusoft)  G003.00.0  issue課題#1836を対応します.
 */
...
export default {
  data () {
    ...
    return {
      ...
      storeId: null,
      headers: [
        { text: '企画コード', value: 'planningCode', sortable: false, width: 100 },
        { text: '企業コード', value: 'companyCode', sortable: false, width: 150 },
        { text: '店舗コード', value: 'storeCode', sortable: false, width: 150 },
        { text: '開始日', value: 'startDateTime', sortable: false, width: 150 },
        { text: '終了日', value: 'endDateTime', sortable: false, width: 150 },
        { text: 'ロゴファイル名', value: 'headerLogoFileName', sortable: false, width: 180 },
        { text: '商品明細', value: 'itemDetails', sortable: false, width: 100 },
        { text: '税区分', value: 'taxClassification', sortable: false, width: 100 },
        { text: '売変マーク', value: 'sellingPriceChangeMark', sortable: false, width: 100 },
        { text: 'その他付加情報エリア', value: 'receiptInformation', sortable: false, width: 200 },
        { text: '領収証その他付加情報エリア', value: 'ryosyuInformation', sortable: false, width: 200 },
        { text: '領収証メッセージ（1行目）', value: 'ryosyuMessage_1', sortable: false, width: 200 },
        { text: '領収証メッセージ（2行目）', value: 'ryosyuMessage_2', sortable: false, width: 200 },
        { text: '領収証メッセージ（3行目）', value: 'ryosyuMessage_3', sortable: false, width: 200 },
        { text: '領収証メッセージ（4行目）', value: 'ryosyuMessage_4', sortable: false, width: 200 },
        { text: '領収証メッセージ（5行目）', value: 'ryosyuMessage_5', sortable: false, width: 200 },
        { text: '領収証メッセージ（6行目）', value: 'ryosyuMessage_6', sortable: false, width: 200 },
        { text: '領収証メッセージ（7行目）', value: 'ryosyuMessage_7', sortable: false, width: 200 },
        { text: '領収証メッセージ（8行目）', value: 'ryosyuMessage_8', sortable: false, width: 200 },
        { text: '領収証メッセージ（9行目）', value: 'ryosyuMessage_9', sortable: false, width: 200 },
        { text: '領収証メッセージ（10行目）', value: 'ryosyuMessage_10', sortable: false, width: 200 },
        { text: '領収証メッセージ（11行目）', value: 'ryosyuMessage_11', sortable: false, width: 200 },
        { text: '領収証メッセージ（12行目）', value: 'ryosyuMessage_12', sortable: false, width: 200 }
  ...
  methods: {
    async initialize () {
      ...
      axios.get(
        this.$i18n.t('prop.url') + ApiPath.ReceiptSetting.ReceiptList + '?companyCode=' + this.corpId + '&storeCode=' + this.storeId,
        ...
        if (result.code === 0) {
          ...
          for (var i = 0; i < response.data.responseModel.length; i++) {
            const startDateTime = response.data.responseModel[i].startDateTime
            const endDateTime = response.data.responseModel[i].endDateTime

            this.presetPlans.push({
              planCd: response.data.responseModel[i].planningCode,
              ...
              sundayFlg: response.data.responseModel[i].sundayFlag,
              mondayFlg: response.data.responseModel[i].mondayFlag,
              tuesdayFlg: response.data.responseModel[i].tuesdayFlag,
              wednesdayFlg: response.data.responseModel[i].wednesdayFlag,
              thursdayFlg: response.data.responseModel[i].thursdayFlag,
              fridayFlg: response.data.responseModel[i].fridayFlag,
              saturdayFlg: response.data.responseModel[i].saturdayFlag
            })
          }
    ...
    initData (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        const startDateTime = items[i].startDateTime
        const endDateTime = items[i].endDateTime

        this.presetPlans.push({
          planCd: items[i].planningCode,
          dateFrom: moment(startDateTime).format('YYYYMMDD'),
          timeFrom: moment(startDateTime).format('hhmm'),
          dateTo: moment(endDateTime).format('YYYYMMDD'),
          timeto: moment(endDateTime).format('hhmm'),
          sundayFlg: items[i].sundayFlag,
          mondayFlg: items[i].mondayFlag,
          tuesdayFlg: items[i].tuesdayFlag,
          wednesdayFlg: items[i].wednesdayFlag,
          thursdayFlg: items[i].thursdayFlag,
          fridayFlg: items[i].fridayFlag,
          saturdayFlg: items[i].saturdayFlag
        })
      }
    },
    // G003.00.0 Add-End
    ...
    getWeekdays (item) {
      let str = ''
      if (item.mondayFlag) str += '月曜、'
      if (item.tuesdayFlag) str += '火曜、'
      if (item.wednesdayFlag) str += '水曜、'
      if (item.thursdayFlag) str += '木曜、'
      if (item.fridayFlag) str += '金曜、'
      if (item.saturdayFlag) str += '土曜、'
      if (item.sundayFlag) str += '日曜、'

      return str.slice(0, -1)
    },
    getPlanCodeFilledInZero (planCd) {
      // 企画は文字リソースから取得する
      return this.$i18n.t('F00002.S096') + ('00' + planCd).slice(-2)
    },
    /**
     * 日付情報を返す
