...
<script type="text/javascript" src="./../../static/js/CorporateMaster/corporateMaster.js"></script>
<template>
  <v-container
      ...
      <v-col class="bkColorBlue">
        <div class="underLine2">
          <input
            type="text"
            class="inputText"
            v-model="businessUnitCdData"
            ref="businessUnitCdText"
            @keydown.enter="directInput"
            @input="businessUnitCdInput"
            style="ime-mode:disabled;"
            :placeholder="this.$i18n.t('F00004.S029')"
            maxlength="15"
    ...
    <v-row
      ...
      <v-col
        ...
        style="padding: 0px;">
        <label class="inputLabel">{{ $t("F00004.S002") }}</label>
        <div class="inline-radio">
          <div><input
            ...
            @change="onRadioChange"><label
              ...
              class="scrollNone">{{ $t("F00004.S003") }}</label></div>
          <div style="margin-left:10px"><input
            ...
            @change="onRadioChange"><label
              ...
              class="scrollNone">{{ $t("F00004.S004") }}</label></div>
        </div>
      </v-col>
      <v-col>
        <div class="underLine2">
    ...
    <v-row style="height: 38px;width: 680px;margin-top:-8px">
      <v-col style="text-align: left;">
        <font
          class="baseFont"
          style="font-size:20px;margin-left:-12px"><b style="font-weight:normal;">{{ $t("F00004.S005") }}{{ resultCount }}{{ $t("F00004.S006") }}</b></font>
      </v-col>
    </v-row>
    <v-row style="height: 30px;width: 680px;">
      <v-col
        :cols="4"
        style="padding: 0px;">
        <label id="ListHeader">{{ $t("F00004.S007") }}</label>
      </v-col>
      <v-col
        :cols="7"
        style="padding: 0px;">
        <label id="ListSplitHeader">{{ $t("F00004.S008") }}</label>
      </v-col>
      <v-col
        :cols="1"
        style="padding: 0px;">
        <label id="ListSplitHeader"><font style="margin-left:-21px">{{ $t("F00004.S009") }}</font></label>
      </v-col>
    </v-row>
    <v-row
      v-if="operationLock === false"
      v-for="(corporateItem,index) in dispCorporateDataList"
      :key="corporateItem.name"
      ...
      <v-col
        ...
        <label
          ...
          :id ="'code'+index"><font
            class="NumericStyle"
            style="margin-left:10px">{{ corporateItem.name }}</font></label>
      </v-col>
      <v-col
        :cols="7"
        style="padding: 0px;">
        <label
          ...
          :id ="'name'+index"><font
            class="NameClass"
            v-if="corporateItem.displayName">{{ corporateItem.displayName.default }}</font></label>
      </v-col>
      <v-col
        :cols="1"
        style="padding: 0px;"
